...
function preload() {
  ...
  // flowerObj = loadModel('flower-bush.obj');
}
...
function draw() {
  ...
  for (planeX = -max; planeX < max; planeX += 2.5 * scl) {
    for (planeY = -max; planeY < max; planeY += 2.5 * scl) {
      push();
      translate(planeX, planeY);
      let xoff = map(planeX, 0, scl * 15, -PI, PI);
      let yoff = map(planeY, 0, scl * 15, -PI, PI);
