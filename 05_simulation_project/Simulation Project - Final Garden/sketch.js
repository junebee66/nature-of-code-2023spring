...
function preload() {
  loadTable("data/dragon-1.csv", "csv", createParticleSystem);
  loadTable("data/bunny-color.csv", "csv", createParticleSystem);
    loadTable("data/terrian-coordinate.csv", "csv", createParticleSystem);
  
      loadTable("data/flower-bush.csv", "csv", createParticleSystem);
...
function createParticleSystem(table) {
  ...
  for (let c = 0; c < table.getRowCount(); c += reductionFactor) {
  let r = parseFloat(table.get(c, 3));
    console.log("r=" + r);
  let g = parseFloat(table.get(c, 4));
  let b = parseFloat(table.get(c, 5));
    table.setNum(c, 3, r);
    table.setNum(c, 4, g);
    table.setNum(c, 5, b);
  }
